...
class DijkstraRouteStrategy(BaseRouteStrategy):
    """
    Implementación concreta de la estrategia de cálculo de rutas usando
    el algoritmo de Dijkstra a través de NetworkX.
    ...
    def calculate(self, road: 'RoadNetwork', origin_id: int, dest_id: int) -> List['ScoredRoute']:
        """
        Calcula tres rutas candidatas (tiempo, distancia, seguridad) y las
        puntúa con un score global ponderado.
        ...
        if route_tiempo:
            candidatos.append((route_tiempo, "Más rápida"))
        ...
        if route_dist and (not candidatos or route_dist.nodes != candidatos[0][0].nodes):
            candidatos.append((route_dist, "Más corta"))
        ...
        if route_segura and route_segura.nodes not in [c[0].nodes for c in candidatos]:
            candidatos.append((route_segura, "Más segura"))
        ...
        for (ruta, label), penalizacion in zip(candidatos, penalizaciones):
            # Score 1.0 = mejor (menor penalización), 0.0 = peor
